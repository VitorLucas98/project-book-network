...
export class BorrowedBookListComponent implements OnInit {
  ...
  private findAllBorrowedBooks() {
    this.bookService.findAllBorrowedBooks(this.page, this.size)
    .subscribe({
      next: (resp) => {
  ...
  returnBook(withFeedback: boolean) {
    this.bookService.returnBorrowBook(this.selectedBook?.id as number)
    .subscribe({
      next: () => {
        if (withFeedback) {
          this.giveFeedback();
  ...
  private giveFeedback() {
    this.feedbackService.saveFeedback(this.feedbackRequest)
    .subscribe({
      next: () => {
      }
    });
